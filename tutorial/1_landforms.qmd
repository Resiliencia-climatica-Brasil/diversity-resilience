---
title: Variedade de formas de relevo (*landforms*)
jupyter: python3
---
...
### Definição da metodologia para calcular as formas de relevo

A metodologia utilizada para definir as formas de relevo (do inglês, *landforms*) foi aquela proposta por Anderson *et al.* (2016). Nós testamos ainda as metodologias propostas por Theobald *et al.* (2015) e Iwahashi & Yamazaki (2022), mas decidimos seguir com aquela proposta por Anderson *et al.* (2016) por ***tais e tais razões***. As formas de relevo geradas a partir da metodologia de Anderson et al. (2016) foram: 
...
![](figura/landform_scheme.png)
    ...
    Figura 1. Classificação de formas de relevo utilizada pela *The Nature Conservancy* para as paisagens na América do Norte. Adaptação de Anderson *et al.* (2016).
...
Em seguida, as classes foram combinadas pela soma de cada classe multiplicada por um peso. O índice de umidade foi multiplicado por 1000, exposição por 100, TPI por 10 e declividade por 1. Desta forma, o número resultante representa um código descrevendo as classes de cada variável. Por exemplo, 1231 é a classe 1 de índice de umidade, 2 de exposição, 3 de TPI e 1 de declividade. Posteriormente, os valores finais foram convertidos em tipos de formas de relevo, seguindo a classificação indicada na Tabela 2.
    ...
    Tabela 2. Critério de conversão dos códigos da combinação de classes em tipos de formas de relevo.
    
|Código|Forma de relevo|
|:-:|:-:|
|10|11|
|11|11|
|12|11|
...
Foi utilizado o MapBiomas ou o MapBiomas Água? Acho que são produtos diferentes, não?

-->

### Códigos criados no *Google Earth Engine*

As análises foram rodadas no *Google Earth Engine* (Gorelick 2017), em função da demanda computacional do projeto. usamos o pacote **geemap** (Wu 2020) em *Python* (Python Software Foundation 2023) como interface, pela facilidade na documentação e reprodutividade das análises. 


### Gerando as variáveis 
<!---

Sugiro seguir a mesma ordem dos itens abaixo conforme ordem citada do no texto: declividade, TPI, exposição e índice de umidade.
Outra coisa, nas explicações do código em Python, seria bom editar para os nomes em português, conforme as edições feitas no texto. Não editei nada dentro do código.

E mais uma coisa: o TPI não seguiu a metodologia do Anderson, certo? Temos que indicar lá em cima que ajustamos a metodologia sugerida por Anderson, substituindo o índice xx pelo TPI e justificar o porquê.

-->

#### Declividade (*slope*)

Criamos a superfície de declividade a partir do Merit-DEM.
...
Calculamos exposição utilizando o mesmo DEM.
...
Calculamos o Índice de Posição Topográfica (TPI) (Weiss 2001) para cada célula do *raster* dentro de um *kernel* circular com 7, 11 e 15 células de raio. O TPI é a diferença média de elevação entre a célula focal e um conjunto de células vizinhas.

$TPI = \frac{\sum_{i}^{n}(vizinhança_i -  focal)}{n}$

a vizinhança *i* representa cada uma das *n* células dentro do *kernel* da célula focal. O índice final é composto pela média de TPI das três janelas, o que permite a consideração de diferentes níveis de resolução da paisagem, tanto local quanto regional (Theobald *et al*. 2015). Os tamanhos das janelas foram definidos visualmente para que melhor representassem as formas de relevo, principalmente os *Summits*, *Valleys*, *Toeslopes* e *Hilltops* (*flat* e *gentle*). Os tamanhos das janelas também tinham que capturar os Platôs como *Summits*. 
...
em que o fluxo é o acúmulo de fluxo e *slope* é o *slope* calculado anteriormente. O *moisture index* é a média do índice dentro de um *kernel* circular de uma célula de raio. O tamanho do raio foi escolhido visualmente para suavizar o índice, mas representando bem a distribuição dos cursos d'água.
...
A exposição foi selecionada para definir as faces norte e sul do relevo no hemisfério sul.

<!---

Não seria a frase abaixo?
"A exposição foi selecionada para definir as faces quente e fria do relevo no hemisfério sul."
...
As classes de TPI foram definidas para representarem bem os *Summits*, *Valleys*, *Toeslopes* e *Hilltops*, que foram as formas de relevo mais difíceis de ajustar os parâmetros.
...
O limiar do índice umidade para classificar as áreas como úmidas ou secas foi definido visualmente para capturar a distribuição dos cursos d'água sem criar áreas planas secas com excesso de ramificações dendríticas.

Grandes rios (ex. Rio Amazonas, represas e lagos) não foram bem representados por essa classificação, pois ela classificava somente a partes mais profundas como áreas úmidas, mantendo o restante dos grandes corpos d'água como regiões planas secas. Nós corrigimos essa classificação combinando a área úmidade classificada pelo acúmulo de fluxo com a camada de água produzida pelo MapBiomas.

<!---

Essa descrição não estava muito boa. Veja se a forma como eu editei está correta.
...
### Classificando os tipos de formas de relevo

Classificamos os tipos de formas de relevo pelo código gerado anteriormente e ajustamos visualmente alguns códigos para melhor representação. Por exemplo, o código 11 representa áreas de baixa declividade e uma posição do relevo mais alta que o entorno, sendo portanto um topo de montanha (*Summit*). No entanto, alguns códigos tiveram que ser bem inspecionados para separar alguns tipos de formas de relevo, como foi o caso de *Sideslopes* em relação a *Valleys* e *Toeslopes*. 
...
Na figura, incluir legenda com a indicação das formas de relevo.
...
A variedade de formas de relevo foi calculada como a soma de tipos diferentes de formas de relevo dentro de um *kernel* circular de uma célula focal. O tamanho do raio do *kernel* foi definido calculando a variedade em diferentes raios (2, 5, 7, 10, 15, 20 células) e calculando o ganho de variedade a cada aumento de raio. O raio escolhido foi aquele no qual o subsequente não adicionou variedade. Desta forma, o raio representa o nível de resolução da paisagem que captura o máximo de variedade de formas de relevo. Raios maiores podem aumentar a variedade, mas devido à mudança de paisagem. Assim, o raio escolhido foi de 5 células de raio (450 metros) para todo o Brasil. Abaixo está uma representação da variedade de formas de relevo para a mesma região de Alto Paraíso de Goiás-GO.
